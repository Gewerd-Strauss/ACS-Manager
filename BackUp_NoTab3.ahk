...
LibraryFile:=A_ScriptDir "\AHK_LibraryGUI.txt"
CurrentMode:="Instr"
...
(LTrim
	...
	GUI requires:
	- add snippet
		-		- give name, long description, short description,examples, code, section
	- edit snippet	
		-     	- rename 
...
f_FillFields(Data)
{
	...
	RC.Value := []
	d:=
	RC.Value:=RegExReplace(Data.code,"`r`n\\\\\\---NewSnippet---\\\\\\`r`n")
...
fParseArr(Arr,SettingsIdentifier,ArrSnippetStrings)
{
    ...
    {   
        ...
        ; ; if snippet.Code
        ; {
        ;     ; Snippets[sCurrentFunc,Code]:=CurrSnippetCode
        ; }
    }
    for k,v in Snippets
    {
