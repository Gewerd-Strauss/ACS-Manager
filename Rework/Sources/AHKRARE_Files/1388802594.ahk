m(x*){
	...
	WinGetTitle,Title,A
	for a,b in x{
		Obj:=StrSplit(b,":"),(Obj.1="Bottom"?(Bottom:=1):""),(VV:=List[Obj.1,Obj.2])?(Value+=VV):(List[Obj.1]!="")?(List[Obj.1]:=Obj.2):TXT.=(IsObject(b)?m_Obj2String(b,,Bottom):b) "`n"
	}
...
m_Obj2String(Obj,FullPath:=1,BottomBlank:=0){
	static String,Blank
	if(FullPath=1)
	...
	if(IsObject(Obj)){
		for a,b in Obj{
			if(IsObject(b))
				m_Obj2String(b,FullPath "." a,BottomBlank)
			else{
				if(BottomBlank=0)
